...
local function kick_ban_res(extra, success, result)
      ...
      elseif get_cmd == 'banlife' then
        send_large_msg(receiver, 'User @'..member..' ['..member_id..']banned from all groups')
		banall_user(member_id)
      elseif get_cmd == 'unbanlife' then
        send_large_msg(receiver, 'User @'..member..' ['..member_id..']unbanned from all groups')
...
if matches[1]:lower() == 'kick' then
  ...
  if matches[1]:lower() == 'banlife' and is_admin1(msg) then -- Global ban
  ...
  if matches[1]:lower() == 'unbanlife' then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'User ['..user_id..' ]unbanned from all groups'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
  ...
  if matches[1]:lower() == "banlifelist" then -- Global ban list
    return banall_list()
  end
end
